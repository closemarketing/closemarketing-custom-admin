...
class WP_Resize_Images_Before_Upload {
    ...
    function __construct() {
        ...
        if ( RIBU_FRONTEND_JS == true ) {
            add_action('wp_footer',  array($this,'cmk_print_js'),10); // it's possible to have the media manager on the front end too, but you should ask for it :)
        }
    ...
    function admin_init_settings(){
        ...
        add_settings_field('cmk_cancel_force_flash',
            'Disable force flash',
    ...
    function resize_quality_callback_function(){
        echo '<input name="cmk_resize_quality" id="cmk_resize_quality" type="text" value="'. $this->get_resize_quality() .'" class="small-text" /> <em class="description">1 - 100   (a low quality value will result in a considerably smaller file size and lower quality images - 80 is optimum)</em>';
    }
    
    function resize_width_callback_function(){
        echo '<input name="cmk_resize_width" id="cmk_resize_width" type="text" value="'. $this->get_resize_width() .'" class="small-text" /> <em class="description">you can override this by setting RIBU_RESIZE_WIDTH in your wp-config file</em>';
    }
    
    function resize_height_callback_function(){
        echo '<input name="cmk_resize_height" id="cmk_resize_height" type="text" value="'. $this->get_resize_height() .'" class="small-text" /> <em class="description">you can override this by setting RIBU_RESIZE_HEIGHT in your wp-config file</em>';
    }
    
    function cancel_force_flash_callback_function(){
        echo '<input name="cmk_cancel_force_flash" id="cmk_cancel_force_flash" type="checkbox" value="1" ' . checked( 1, get_option('cmk_cancel_force_flash'), false ) . ' class="small-text" /> <em class="description">Do not force the Flash uploader for non Chrome/Firefox browsers.</em>';
    }
